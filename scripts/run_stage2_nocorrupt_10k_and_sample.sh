...
# Retrain stage-2 for 10k steps with corrupt_mode=none (match inference),
# then sample with the new checkpoint.
# Usage:
#   bash scripts/run_stage2_nocorrupt_10k_and_sample.sh
#
...
CKPT_DIR=${CKPT_DIR:-checkpoints/interp_levels_unified_T128_dp_nocorrupt_10k}
LOG_DIR=${LOG_DIR:-runs/interp_levels_unified_T128_dp_nocorrupt_10k}
OUT_ROOT=${OUT_ROOT:-runs/ablate_stage2_unified_dp_nocorrupt_10k}
...
echo "==> Train stage-2 (corrupt_mode=none, 10k steps)"
"${PYTHON_BIN}" -m src.train.train_interp_levels \
  ...
  --corrupt_mode none --corrupt_sigma_max 0.0 --corrupt_sigma_min 0.0 --corrupt_sigma_pow 1.0 \
  --corrupt_anchor_frac 0.0 --corrupt_index_jitter_max 0 --corrupt_index_jitter_prob 0.0 --corrupt_index_jitter_pow 1.0 \
