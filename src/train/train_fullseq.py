...
def build_argparser():
    ...
    p.add_argument("--log_dir", type=str, default="runs/fullseq")
    p.add_argument("--ckpt_dir", type=str, default="checkpoints/fullseq")
...
def main():
    ...
    for step in pbar:
        ...
        t = torch.randint(0, args.N_train, (x.shape[0],), device=device, dtype=torch.long)
        ...
        with torch.cuda.amp.autocast(dtype=autocast_dtype):
            ...
            diff = diff.sum(dim=-1)
            mask_f = (~mask).float()
            loss = (diff * mask_f).sum() / (mask_f.sum() * x.shape[-1] + 1e-8)
